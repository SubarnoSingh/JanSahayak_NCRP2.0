/**
 * Seed script — synthetic demo data only. No real PII.
 * Run: npm run seed
 */
...
async function seed(): Promise<void> {
  ...
  const resources = [
    ...
    },
    {
      ...
      body: [
        "## What is a Boss Scam?",
        "A Boss Scam (also called CEO Fraud or Business Email Compromise) is when a fraudster impersonates a senior executive — such as your CEO, CFO, or a trusted business partner — and sends you files or messages that look official. These files often contain malware designed to steal your credentials, access your bank accounts, or compromise your device.",
        "",
        "## How the scam works",
        "1. The scammer researches your company or contacts and identifies senior figures (CEO, director, auditor).",
        ...
        "4. The file is disguised as an account statement, MCA filing, RBI notice, tax document, or compliance report.",
        ...
        "- The email comes from a slightly different address (e.g., ceo@company-name.com instead of ceo@companyname.com).",
        ...
        "- The tone is formal but lacks specific details you would normally see from the real person.",
        "- You are asked to keep the request confidential or bypass normal approval processes.",
        "",
        "## How malicious files can be dangerous",
        "- PDFs and Office documents can contain embedded scripts that install spyware or keyloggers.",
        "- Macro-enabled Excel sheets can execute code the moment the file is opened.",
        "- Fake invoice files may contain links to phishing sites that steal your banking credentials.",
        "- Some files exploit vulnerabilities in outdated software to gain access to your device.",
        "",
        "## What you should never do",
        "- Never open attachments from unknown or unexpected senders — even if they appear to come from a known contact.",
        "- Never enable macros or content in Office documents unless you are certain the file is genuine.",
        "- Never enter passwords or OTPs on any page reached through a link in a received document.",
        "- Never transfer funds based solely on an email instruction, even if it appears to come from a senior executive.",
        "",
        "## How to verify a request",
        "- Call the supposed sender on their known phone number — not the number in the suspicious email.",
        ...
        "- If the file claims to be from a government portal, verify it by visiting the official website directly.",
        "",
        "## What to do if a suspicious file was opened",
        "- Disconnect the device from the internet immediately to prevent data exfiltration.",
        "- Run a full antivirus and malware scan on the device.",
        "- Change passwords for any accounts accessed from that device, starting with email and banking.",
        "- Inform your IT department or security team right away.",
        "- If financial data was exposed, contact your bank to freeze or monitor accounts.",
        "",
        "## How to report the incident",
        "- File a complaint on this portal with the email, file, and sender details as evidence.",
        "- Call 1930 if money was transferred.",
        "- Save the original email (including full headers), the attached file, and any related messages.",
        "- Report the sender's email address to your email provider for phishing.",
      ].join("\n"),
    ...
    },
    {
      ...
      body: [
        ...
        "- Never install remote-access apps (AnyDesk, RustDesk etc.) for 'customer support'.",
  ...
  /* ── Demo incident (submitted, financial fraud) ─────── */
  const narrative =
  ...
  // Attach synthetic evidence meta + money trail to the demo case
